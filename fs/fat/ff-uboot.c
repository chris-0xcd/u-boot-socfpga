...
static struct disk_partition *fat_part;
...
int fat_set_blk_dev(struct blk_desc *dev, struct disk_partition *part)
{
	FRESULT ff_ret;
...
int fat_unlink(const char *filename)
{
    ...
    ff_ret = f_unlink(filename);
    if (ff_ret != FR_OK) {
        debug("f_unlink() failed: %d\n", ff_ret);
...
int fat_mkdir(const char *dirname)
{
    ...
    ff_ret = f_mkdir(dirname);
    if (ff_ret != FR_OK) {
        debug("f_mkdir() failed: %d\n", ff_ret);
...
int fat_uuid(char *uuid_str)
{
    ...
    if (f_getlabel("0:", NULL, (DWORD*)&vsn) != FR_OK) {
        return -1;
    }

    sprintf(uuid_str, "%02X%02X-%02X%02X", vsn[3], vsn[2], vsn[1], vsn[0]);
